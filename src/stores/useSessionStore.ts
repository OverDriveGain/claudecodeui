...
async function requestSessionHistoryPage(
  ...
  // MYMU (F2): route by session ownership so a peer-user/peer-host agent's
  // transcript is fetched from the host that owns it, with THAT user's token —
  // not the primary login (which may be scoped out of the agent → empty history).
