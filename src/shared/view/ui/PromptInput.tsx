    ...
    return (
      <PromptInputContext.Provider value={contextValue}>
        <form
          ...
          className={cn(
            'relative overflow-hidden rounded-3xl border border-border/50 bg-card/80 shadow-sm backdrop-blur-sm transition-all duration-200 focus-within:border-primary/30 focus-within:shadow-md focus-within:ring-1 focus-within:ring-primary/15',
            className
          )}
          {...props}
        >
...
>(({ className, ...props }, ref) => (
  <textarea
    ...
    className={cn(
      'chat-input-placeholder block max-h-[40vh] w-full resize-none overflow-y-auto bg-transparent px-5 py-3.5 font-sans text-[17px] leading-6 text-foreground placeholder-muted-foreground/50 focus:outline-none sm:max-h-[300px]',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn('flex items-center justify-between px-4 pb-3 pt-1', className)}
    {...props}
  />
));
