...
export const sessionsService = {
  ...
  async fetchHistory(
    ...
    // Project-restricted users may read history only for local projects their
    // project_allow (which inherits agent_allow) matches. Deny others rather than
    // leak that the session exists.
    if (currentProjectAllow()?.length && !isNameAllowedForUser(path.basename(session.project_path ?? ''))) {
      return { messages: [], total: 0, hasMore: false, offset: 0, limit: null };
    }
