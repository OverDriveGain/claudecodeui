...
// MYMU: multi-user spawn-as-owner (FORK.md S2) — a project owned by a mapped
...
// MYMU: writes (once per boot) the shell wrapper that execs the claude CLI as
// a mapped foreign linux user. Params ride in env (MYMU_SPAWN_USER/CWD) because
// the SDK owns the argv; the target user's own `claude` resolves inside their
// login shell. Requires sudoers: <service> ALL=(%mymu-users) NOPASSWD: ALL.
let mymuSpawnWrapperPath = null;
function ensureSpawnAsUserWrapper() {
  if (mymuSpawnWrapperPath) return mymuSpawnWrapperPath;
...
function mapCliOptionsToSDK(options = {}) {
  ...
  // MYMU: foreign-owned project → sudo wrapper; the wrapper enters the project
  // dir only after the identity switch (the service user often cannot traverse
  // that home), so the SDK itself spawns from a neutral cwd.
  if (options.spawnOwner) {
    sdkOptions.env.MYMU_SPAWN_USER = options.spawnOwner;
    sdkOptions.env.MYMU_SPAWN_CWD = cwd || `/home/${options.spawnOwner}`;
...
async function queryClaudeSDK(command, options = {}, ws, context) {
  ...
  // MYMU: resolve the project's owning linux user; scoped users may only spawn
  // inside their own mapped home (closes the crafted-cwd hole), and foreign-
  // owned projects run as their owner via the sudo wrapper.
  let mymuSpawnOwner = null;
  try {
    const owner = options.cwd ? ownerForPath(options.cwd) : null;
    ...
    console.warn('[Claude SDK] MYMU owner resolution failed:', error?.message || error);
  }
  if (mymuSpawnOwner) options = { ...options, spawnOwner: mymuSpawnOwner };
  ...
  try {
    ...
    const mcpServers = mymuSpawnOwner ? null : await loadMcpConfig(options.cwd); // MYMU: foreign homes unreadable
    if (mcpServers) {
