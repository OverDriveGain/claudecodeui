      ...
      if (Array.isArray(raw.message.content)) {
        ...
        for (let partIndex = 0; partIndex < raw.message.content.length; partIndex++) {
          ...
          if (part.type === 'tool_result') {
            ...
            // MYMU merge: upstream's files-input parsing + MyMu's user-row
            // normalization (command records, stdout, task notifications,
            // injected classification); attachments ride on the first user row.
            const text = part.text || '';
            const parsedFiles = parseFilesInputTag(text);
            const rows = normalizeUserTextRow(parsedFiles.text, `${baseId}_text_${partIndex}`, sessionId, ts, injected);
            const attachTo = rows.find((m) => m.kind === 'text' && m.role === 'user');
            if (attachTo) {
              if (!imagesAttached && imageAttachments.length > 0) {
                attachTo.images = imageAttachments;
                imagesAttached = true;
              }
              if (!filesAttached && parsedFiles.attachments.length > 0) {
                attachTo.files = parsedFiles.attachments;
                filesAttached = true;
              }
            } else if (!filesAttached && parsedFiles.attachments.length > 0) {
              // Files-only send: still deserves a user bubble.
              rows.push(createNormalizedMessage({
                id: `${baseId}_files_${partIndex}`,
                sessionId,
                timestamp: ts,
                provider: PROVIDER,
                kind: 'text',
              ...
              filesAttached = true;
              if (imageAttachments.length > 0) imagesAttached = true;
            }
            messages.push(...rows);
          }
        }
        ...
        if (messages.length === 0) {
          const textParts = raw.message.content
            ...
            .filter(Boolean)
            .join('\n');
          // MYMU merge: normalized rows + upstream's image attachment on the
          // first plain user row.
          const rows = normalizeUserTextRow(textParts, `${baseId}_text`, sessionId, ts, injected);
          const attachTo = rows.find((m) => m.kind === 'text' && m.role === 'user');
          if (attachTo && imageAttachments.length > 0) {
            attachTo.images = imageAttachments;
            imagesAttached = true;
          }
          messages.push(...rows);
        ...
        // MYMU merge: strip upstream's files-input tag, route the remaining
        // text through the shared user-text normalizer (slash commands, local
        // stdout, task notifications, injected payloads), and attach parsed
        // files to the resulting user row.
        const parsedFiles = parseFilesInputTag(text);
        const rows = normalizeUserTextRow(parsedFiles.text, baseId, sessionId, ts, injected);
        const fileTarget = rows.find((m) => m.kind === 'text' && m.role === 'user');
        if (fileTarget && parsedFiles.attachments.length > 0) {
          fileTarget.files = parsedFiles.attachments;
        } else if (!fileTarget && parsedFiles.attachments.length > 0) {
          rows.push(createNormalizedMessage({
            id: `${baseId}_files`,
            sessionId,
            timestamp: ts,
